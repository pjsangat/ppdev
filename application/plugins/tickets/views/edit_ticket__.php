...
<form style='margin:0px'  method="post" action="index.php?c=task&a=add_list&active_project=<?php echo $_GET['active_project'];  ?>&<?php echo time()?>">
<input type="hidden" value="<?php echo htmlentities(array_var($ticket_data, 'summary')) ?>" name="task_list[name]"> 
<input type="hidden" value="" name="task_list_start_date">
<input type="hidden" value="" name="task_list_due_date">
<input type="hidden" value="" name="task_list[priority]">
<input type="hidden" value="<?php echo htmlentities($ticket->getDescription()); ?>" name="task_list[description]">
<input type="hidden" value="<?php echo htmlentities(array_var($ticket_data, 'milestone_id')); ?>" name="task_list[milestone_id]">


<input type="hidden" value="<?php echo htmlentities($ticket->getDescription()); ?>" name="task_list[task0][text]">

<?php /* <textarea name="task_list[task0][text]"><?php echo htmlentities(render_object_comments($ticket, $ticket->getViewUrl())) ?></textarea> */ ?>

<input type="hidden" value="<?php echo htmlentities(array_var($ticket_data, 'assigned_to')); ?>" name="task_list[task0][assigned_to]">

<input type="submit" value="Convert Ticket As Task List">
</form>

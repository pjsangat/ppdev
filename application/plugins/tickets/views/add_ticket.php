<?php 
  ...
  add_stylesheet_to_page('project/tickets.css');
	...
	if(!active_project()){
		echo "<script>self.location='index.php?c=tickets&a=add_ticket&active_project=1&".time()."';</script>";
		exit();
	}
	else{
		...
		function changeProject(){
			active_project = jQuery("#project").val();
			action = "index.php?c=tickets&a=add_ticket&active_project="+active_project;
			jQuery("#add_ticket_form")[0].action = action;
		}
		</script>
		<form action="<?php echo get_url('tickets', 'add_ticket') ?>" method="post" enctype="multipart/form-data" id='add_ticket_form'>
		
		<?php tpl_display(get_template_path('form_errors')) ?>
		
		
		  <?php
		  if(active_project()->getId()==1){
			  ?>
			  <div>
				<?php echo label_tag("Project", 'project', true) ?>
				<?php 
					$projects = Projects::getAll();
					?><select id='project' onchange='changeProject()'><?php
					foreach($projects as $project){
						if(strtolower($project->getName())!="welcome"){
							?><option value='<?php echo $project->getId(); ?>'><?php echo $project->getName(); ?></option><?php
						}
					}
					?></select><?php
				?>

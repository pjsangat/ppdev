...
class EMAIL_TO_DB {
 ...
 var $file_path = '/var/www/files/email/'; #Where to write file attachments to /var/www/files/email/
 ...
 function connect($host, $port, $login, $pass){
  ...
  $this->mailbox = md5($host.$login.$pass);
  //get latest message id that was parsed
  $sql = "select * from `emailtodb_email` where `Mailbox`='".$this->mailbox."' order by IDEmail desc  limit 1";
  $execute = mysql_query($sql);
  $row = mysql_fetch_array($execute);

  $this->msgid = $row['IDEmail']+1;
...
*/
  function db_add_message($email){
	if($email['MESSAGE_NO']){
		$execute = mysql_query("INSERT INTO emailtodb_email (Mailbox, IDEmail, EmailFrom, EmailFromP, EmailTo, DateE, DateDb, Subject, MsgSize) VALUES
		  (
		  '".$this->mailbox."',
		  '".$this->msgid."',
		  '".$email['FROM_EMAIL']."',
 ...
 */
  ...
  function do_action(){
           ...
           $result = imap_search($this->link, 'UNSEEN');
          $this->msgid =$result[0];
...
//                pe($email);
               $emailChunk = explode('@', $email['FROM_EMAIL']);

                          while($email['MESSAGE_NO']){
                               
//                              if(!empty($emailChunk[0]) && strtolower($emailChunk[0])=='vicki'){
//
//                                   if(!empty($emailChunk[1]) && strtolower($emailChunk[1])== 'adpartners.com'){ //adpartners
//
//                                   }elseif(!empty($emailChunk[1]) && strtolower($emailChunk[1])== 'mailfresh.com'){ //mailfresh
//
//                                   }else{ //default direct open
//
//                                   }

...
<?php if(date("Y", strtotime($time->getDoneDate())) > DateTimeValueLib::now()->getYear()) { ?>
      <div class="doneDate"><span><?php echo lang('done date') ?>:</span> <?php echo format_date($time->getDoneDate(), "Y-m-d H:i:s", 0) ?></div>
<?php } else { ?>
      <div class="doneDate"><span><?php echo lang('done date') ?>:</span> <?php echo format_descriptive_date($time->getDoneDate(), 0, "Y-m-d H:i:s") ?></div>
<?php } // if ?>
